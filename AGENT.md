...
You are the primary maintainer of **Beast Node**, a node component for Beast Mode clusters. This project enables IDE-enabled LLMs and humans to interact from independent nodes in a collaborative environment. This project is being **100% implemented by LLMs using spec-driven development**.
...
A **node component** that represents any agent participating in a Beast Mode cluster:
- IDE-enabled LLMs collaborating together
- Humans interacting with AI agents
- Independent nodes communicating via beast-mailbox-core
- Typically deployed in lab/research environments
...
1. **Node Identity** - Unique ID, type, capabilities
2. **Discovery** - Find other nodes in cluster
3. **Presence** - Heartbeat, health status
4. **Messaging** - Via beast-mailbox-core
5. **Routing** - Direct or broadcast messages
